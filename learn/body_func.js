    ...
    var Sorting = function () {
     ...
     this.bubbleSort = function (callback) {
        ...
        do {
          ...
          state.status = 'Thiết lập biểu đồ <b>đã hoán đổi </b>thành sai.<div>Sau đó lặp lại từ  vị trí  1 đến {endIdx} .</div>'.replace("{endIdx}", indexOfLastUnsortedElement - 1);
          state.lineNo = [2, 3];
          ...
          for (var i = 1; i < indexOfLastUnsortedElement; ++i) {
            ...
            state.status = '<div>So sánh. Nếu  {val1} &gt; {val2} thì hoán đối hai phần tử;&nbsp;<b>hoán đổi </b>= {swapped}.</div>'.replace("{val1}", `${state.backlinks[i - 1].value} (vị trí ${i - 1})`).replace("{val2}", `${state.backlinks[i].value} (vị trí ${i})`).replace("{swapped}", swapped);
            state.lineNo = 4;
            StateHelper.updateCopyPush(statelist, state);
            ...
            if (state.backlinks[i - 1].value > state.backlinks[i].value) {
              ...
              // Set swapped = true.
              state.status = 'Hoán đổi vị trí {idx1} và vị trí {idx2}.'.replace("{idx1}", i - 1).replace("{idx2}", i);
          ...
          if (swapped == false)
            ...
            state.status = 'Các phần tử đã được sắp xếp.<div>Kết thúc thuật sắp xếp nổi bọt</div>';
          else
            // Mark last unsorted element as sorted now.
            // As at least one swap is done in this pass, we continue.
            state.status = '<div>  Đánh dấu phần tử đã được sắp xếp.</div><div>Chúng ta sẽ tiếp tục duyệt cho đến khi trong lần duyệt này không còn hoán đổi nào xảy ra.</div>';
