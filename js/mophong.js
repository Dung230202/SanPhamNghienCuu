        ...
        let e = {
            ...
            render(e) {
                ...
                let r = parseInt(getComputedStyle(t, null).height),
                    ...
                    i = n / 2,
                    o = "rgb(8,103,59)".match(/\d+/g).map(Number),
                    a = "rgb(218,251,111)".match(/\d+/g).map(Number),
                    l = (a[0] - o[0]) / e,
                    s = (a[1] - o[1]) / e,
                    u = (a[2] - o[2]) / e,
                    c = [];
        ...
        let t = {
            async bubble() {
                for (let t = e.length; t >= 0; t--) {
                    let r = !1;
                    for (let n = 0; n < t - 1; n++) await e.highlight(n, n + 1), e[n] > e[n + 1] && (await e.swap(n, n + 1), r = !0);
                    if (!r) return
                }
            },
            ...
            async sleep() {
                ...
                for (let o = 0; o < e.length; o++) {
                    let a = n[o],
                        l = i - (new Date).getTime();
                    setTimeout(i => {
                        if (e.isFrozen()) return;
                        let o = parseFloat(n[0].style.width),
                            l = n[t].style.left;
                        for (let e = t; e < n.length; e++) {
                            if (n[e] === a) {
                                r.insertBefore(a, n[t]), a.style.left = l;
                                break
                            }
                            n[e].style.left = parseFloat(n[e].style.left) + o + "px"
                        }
                        t++
                    }, l + 20 * e[o])
                }
                return new Promise(e => setTimeout(e, 8e3))
            }
        };
        (r => {
